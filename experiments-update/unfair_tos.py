...
class DataTrainingArguments:
    ...
    shuffle_enable: Optional[str] = field(
        ...
        "help": "For shuffling the data that we are going to use to fine tune our model. "
        "By default no shuffle is done. With 'simple_random_shuffle' argument the data are shuffled in random using the given seed "
        "With 'random_shuffle_no_duplicates' argument the data are randomly shuffled and also duplicates within the texts get eliminated"
...
def main():
    ...
    if data_args.shuffle_enable == 'simple_random_shuffle':
        ...
        predict_dataset = predict_dataset.map(updateDatasetTextField,load_from_cache_file= False, desc = "Running shuffler on prediction dataset")
